...
func randomSeekTest(size int64, in1, in2 *os.File, file1, file2 string) {
	...
	if err != nil {
		log.Panicf("Seek failed on %q: %v", file1, err)
	}
	_, err = in2.Seek(start, io.SeekStart)
	if err != nil {
		log.Panicf("Seek failed on %q: %v", file2, err)
	}

	buf1 := make([]byte, blockSize)
	n1, err := io.ReadFull(in1, buf1)
	if err != nil {
		log.Panicf("Read failed on %q: %v", file1, err)
	}

	buf2 := make([]byte, blockSize)
	n2, err := io.ReadFull(in2, buf2)
	if err != nil {
		log.Panicf("Read failed on %q: %v", file2, err)
	}

	if n1 != n2 {
		log.Panicf("Read different lengths %d (%q) != %d (%q)", n1, file1, n2, file2)
	}
	...
	if !bytes.Equal(buf1, buf2) {
		...
		if err != nil {
			log.Panicf("Failed to write /tmp/z1: %v", err)
		}
		err = ioutil.WriteFile("/tmp/z2", buf2, 0777)
		if err != nil {
			log.Panicf("Failed to write /tmp/z2: %v", err)
		}
		log.Panicf("Read different contents - saved in /tmp/z1 and /tmp/z2")
	}
}
...
func main() {
	...
	args := flag.Args()
	if len(args) != 2 {
		log.Panicf("Require 2 files as argument")
	}
	file1, file2 := args[0], args[1]
	in1, err := os.Open(file1)
	if err != nil {
		log.Panicf("Couldn't open %q: %v", file1, err)
	}
	in2, err := os.Open(file2)
	if err != nil {
		log.Panicf("Couldn't open %q: %v", file2, err)
	}

	fi1, err := in1.Stat()
	if err != nil {
		log.Panicf("Couldn't stat %q: %v", file1, err)
	}
	fi2, err := in2.Stat()
	if err != nil {
		log.Panicf("Couldn't stat %q: %v", file2, err)
	}

	if fi1.Size() != fi2.Size() {
		log.Panicf("Files not the same size")
	}
	...
	err = in1.Close()
	if err != nil {
		log.Panicf("Error closing %q: %v", file1, err)
	}
	err = in2.Close()
	if err != nil {
		log.Panicf("Error closing %q: %v", file2, err)
	}
}
