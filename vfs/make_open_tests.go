...
func whichError(err error) string {
	...
	log.Panicf("Unknown error: %v", err)
	return ""
}
...
func test(fileName string, flags int, mode string) {
	...
	if !os.IsNotExist(err) {
		log.Panicf("File must not exist")
	}
	...
	if n != 5 || err != nil {
		log.Panicf("failed to write n=%d: %v", n, err)
	}
	// close
	err = f.Close()
	if err != nil {
	...
	if err != nil && err != io.EOF {
		log.Panicf("failed to read n=%d: %v", n, err)
	}
	err = f.Close()
	if err != nil {
		log.Panicf("failed to close: %v", err)
	...
	if err != nil {
		log.Panicf("failed to remove: %v", err)
	}
